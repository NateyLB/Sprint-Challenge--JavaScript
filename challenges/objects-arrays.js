...
const unisWithUni = [];
graduates.filter((currentValue, index) => {
  var bool = graduates[index].university.search("Uni");
  if(bool != -1){
  unisWithUni.push(graduates[index].university);
  };
});
...
*/
const displayNames = [];
zooAnimals.forEach((element, index) => {
...
lowPopulationAnimals = zooAnimals.filter(element => 
  element.population < 5);
...
populationTotal = zooAnimals.reduce((total, currentValue) => total.population + currentValue.population)
